...
class PersonaSteerer:
    ...
    def apply_and_generate(self, test_prompts, steering_vectors=None, scale=1.0,
                           ...
                           max_new_tokens=200, temperature=0.7, top_k=50, num_beams=1, layers=None):
        """
...
if __name__ == "__main__":
    ...
    print("Layers: ", steerer.model.model.layers)
    ...
    for i, output_toks in enumerate(baseline_outputs):
        ...
        baseline_answers.append(decoded_text)
    ...
    # arrogant_outputs = steerer.apply_and_generate(test_prompts, arrogant_vectors, scale=2, normalise=True, project=False, layers=None)
    # arrogant_answers = []
    # for i, output_toks in enumerate(arrogant_outputs):
    #     decoded_text = steerer.tokenizer.decode(output_toks, skip_special_tokens=True)
    #     arrogant_answers.append(decoded_text)

    # modest_outputs = steerer.apply_and_generate(test_prompts, modest_vectors, scale=2, normalise=True, project=False, layers=None)
    # modest_answers = []
    # for i, output_toks in enumerate(modest_outputs):
    #     decoded_text = steerer.tokenizer.decode(output_toks, skip_special_tokens=True)
    #     modest_answers.append(decoded_text)
    #         # print(f"\nPrompt: {test_prompts[i]}")
    ...
    #         print(f"\nPrompt: {test_prompts[i]}")
    #         print(f"Baseline: {baseline_answers[i].strip()}\n")
    #         print(f"Arrogant: {arrogant_answers[i].strip()}\n")
    #         print(f"Modest: {modest_answers[i].strip()}\n")
    #         print("-" * 50)


    for layer_idx in range(0, len(steerer.model.model.layers), 4):

        print(f"\n--- Generating Responses (Arrogant Persona) {layer_idx} - {layer_idx+4}---")
        # Experiment with scale, `project` often reduces the effect or makes it more nuanced
        arrogant_outputs = steerer.apply_and_generate(test_prompts, arrogant_vectors, scale=2, normalise=True, project=False, layers=range(layer_idx, layer_idx+4))
        arrogant_answers = []
        for i, output_toks in enumerate(arrogant_outputs):
            decoded_text = steerer.tokenizer.decode(output_toks, skip_special_tokens=True)
            arrogant_answers.append(decoded_text)

        print(f"\n--- Generating Responses (Modest Persona) {layer_idx} - {layer_idx+4} ---")
        modest_outputs = steerer.apply_and_generate(test_prompts, modest_vectors, scale=2, normalise=True, project=False, layers=range(layer_idx, layer_idx+4))
        modest_answers = []
        for i, output_toks in enumerate(modest_outputs):
            decoded_text = steerer.tokenizer.decode(output_toks, skip_special_tokens=True)
            modest_answers.append(decoded_text)
        ...
        print("\nSummary of Responses: layers", layer_idx, "to", layer_idx+4)
        for i in range(len(test_prompts)):
            print(f"\nPrompt: {test_prompts[i]}")
            print(f"Baseline: {baseline_answers[i].strip()}\n")
            print(f"Arrogant: {arrogant_answers[i].strip()}\n")
